...
function Get-FmxMetaLockPath {
    ...
    # \A..\z and [regex]::IsMatch, NOT '^..$' with -notmatch, for two reasons that
    # both turn this refusal back into an acceptance:
    #   - .NET's '$' also matches BEFORE a final newline, so an id of "abc`n" -
    #     which bash rejects, `n being outside [A-Za-z0-9._-] - would satisfy
    #     '^[A-Za-z0-9._-]+$' and get a lock path with a newline in it.
    #   - PowerShell's -match is CASE-INSENSITIVE and culture-sensitive, which is
    #     the very thing this module's ordinal-comparison rule exists to keep out
    #     of a gate.
    # bin/fm-promote.ps1's twin of this same bash function spells it the same way.
