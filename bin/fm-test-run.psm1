...
$script:FmTestRunFamilyOrder = @(
    ...
    @{ Family = 'watcher-wake-lock'; Names = @(
            ...
            'fm-wake-queue.test.sh', 'fm-watch-arm.test.sh',
    ...
    @{ Family = 'secondmate'; Names = @(
            'fm-backlog-handoff.test.sh', 'fm-on.test.sh',
            'fm-remote-backlog-handoff.test.sh',
            'fm-remote-doctor.test.sh', 'fm-remote-job.test.sh',
            'fm-remote-job-orphan-reap.test.sh',
            'fm-remote-reply.test.sh', 'fm-remote-secondmate-lifecycle-e2e.test.sh',
            'fm-remote-secondmate-trace-context.test.sh',
            'fm-secondmate-harness.test.sh',
    ...
    @{ Family = 'backend-dispatch'; Names = @(
            ...
            'fm-herdr-session-cleanup.test.sh', 'fm-send-resolve-key.test.sh',
...
function Get-FmTestRunKnownLane {
    ...
    for ($i = 1; $i -le $script:FmTestRunPortableSerialShards; $i++) {
        ...
        # argument list a comma separates ARGUMENTS, so `Add("{0}of{1}" -f $i,
        # $n)` parses as `Add(("{0}of{1}" -f $i), $n)` - the format string is
        # handed one argument, {1} has nothing to bind to, and the whole lane
        # listing dies at run time.
    ...
    switch ($Lane) {
        ...
        default {
            ...
            if ($Lane.StartsWith('portable-serial-', [System.StringComparison]::Ordinal)) {
                ...
                $assignments = Get-FmTestRunPortableSerialAssignment -Context $Context
