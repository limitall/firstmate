...
# Identity of the process that owns an in-flight recovery send: stable for as
# long as that process lives, and different after PID reuse. Mirrors
# bin/fm-wake-lib.sh's fm_pid_identity rather than sourcing it - that lib
# creates its state dir on source, and this one must stay side-effect-free.
# One label is enough here because the two renderings below cannot collide:
# no ps lstart line ever begins "proc-starttime=".
fm_pending_reply_pid_identity() {  # <pid>
  local pid=$1 identity proc_root stat_line starttime cmdline_hex
  local -a stat_fields
  case "$pid" in ''|*[!0-9]*) return 1 ;; esac
  proc_root=${FM_PROC_ROOT_OVERRIDE:-/proc}
  # Prefer a Linux-compatible /proc when present: stat field 22 (starttime, in
  # clock ticks since boot) is immune to the wall-clock steps that re-render the
  # ps lstart fallback's date and would drop a live sender's record into
  # unknown-delivery escalation; the full NUL-separated cmdline keeps PID reuse
  # a mismatch even on a tick collision. Git Bash/MSYS exposes these compatible
  # files but its Cygwin ps rejects the fallback's -o fields outright, so
  # capability detection must not key on uname.
  if [ -r "$proc_root/$pid/stat" ] && [ -r "$proc_root/$pid/cmdline" ]; then
    # `read`, not `cat`: the recovery tick calls this per record, and MSYS -
    # the platform this branch exists for - charges 10-30x Linux per fork.
    stat_line=''
    IFS= read -r stat_line < "$proc_root/$pid/stat" 2>/dev/null || true
  ...
  # which would otherwise mismatch on a non-C locale and drop a live sender.
