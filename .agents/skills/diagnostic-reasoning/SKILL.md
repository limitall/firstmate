...
## One Windows-specific reproduction rule

A bug reported against firstmate-win must be reproduced **on Windows**, in PowerShell 7, in a real checkout.
This port's costliest defects have all been ones a Linux development run could not see: a placeholder file where a symlink should be, a sharing violation on a file another process holds open, a path comparison whose case sensitivity is inverted, a session that had already loaded the PowerShell profile.
Reproducing on the Linux development path and calling it the bug is exactly the mistake that shipped those with a green suite.
When the Windows machine is genuinely unreachable, say so in the report and label the finding unreproduced rather than treating a Linux run as equivalent evidence.
