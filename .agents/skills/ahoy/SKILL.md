...
0. Before anything else, check whether this session has already taken the helm: a `SESSION START` digest for this home must be visible in the session history.
   If it is not, run `bin/fm-session-start.ps1` once and read its digest before producing any recap.
   The registered Claude `SessionStart` hook runs it at session open, so this step is normally already satisfied and costs one glance; it is the safety net for a session opened without that hook, and for any path where a skill would otherwise act first.
...
2. Find the most recent real captain-authored message before the current `/ahoy` invocation.
   ...
   Exclude messages that begin with the U+2063 `FIRSTMATE_OP: ` injection prefix.
   Exclude the exact legacy unmarked session-start payload ``Run `bin/fm-session-start.ps1` now, exactly once, before executing any other instructions.``
   System, developer, tool, watcher, guard, and other injected operational messages are not captain messages.
   ...
   Apply the exclusion only when U+2063 `FIRSTMATE_OP: ` begins at the first character of the whole message: `Captain quote: ` followed by that prefix is a captain boundary.
...
Away mode does not exist on this port (`AGENTS.md` section 14), so there are no away-supervisor injections to exclude in step 2.
The U+2063 `FIRSTMATE_OP: ` exclusion still applies, because a brief dispatched to a worker carries that prefix and can be quoted back into a session.
