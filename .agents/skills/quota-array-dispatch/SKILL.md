...
`AGENTS.md` section 4 owns the always-loaded intake boundary, load trigger, malformed-config refusal, every-candidate accounting, and strongest-reasoning and tie safety rules.
`harness-adapters` owns harness verification, model discovery, and effort fallback.
...
## Two port facts that bound this procedure

1. **`claude` is the only verified harness here** (`harness-adapters`), so a profile array on this machine varies by MODEL and EFFORT, not by harness.
   A candidate naming another adapter is not a tight-quota tradeoff; it is invalid configuration, and bootstrap reports it as such.
   Report that and stop rather than selecting around it.
2. **`quota-axi` is probed, not assumed.**
   Bootstrap prints `MISSING: quota-axi` when it is absent or below its floor.
   With no `quota-axi` output there is no quota evidence at all: say so, and either ask the captain which candidate to use or dispatch the configured default while naming the missing evidence.
   Never invent headroom, and never call a choice quota-informed when nothing was read.

`bin/fm-vendor-auth-probe.sh` is not ported, so the credential ground-truth probe below is unavailable here.
Where the Linux procedure would probe, this port stops at disclosed uncertainty instead: an unproven credential keeps the candidate eligible and is reported as unknown, never converted into a block.
...
For each candidate, preserve explicit `harness`, `model`, and `provider`; model and provider never infer harness:

- task and profile fit and required reasoning class
- applicable effective headroom (`effectivePercentRemaining`) from the established provider or model scope
...
- effective pace, signed reserve per window, and worst reserve for later diagnostic tie-breaking
...
No script maps a model to a provider, a provider to a credential store, or a name prefix to a family.
You establish those relations yourself, in the open, from the candidate's own authoritative catalog (`harness-adapters` owns the discovery surface) plus the one intake snapshot.
...
2. Apply quota at the granularity the vendor actually supplies.
   A provider-level or all-models scope bounds every model you established in that family, including one with no window of its own.
   A named-model scope is an additional bound for that model alone and is irrelevant to every other model in the family.
...
A candidate authenticates through its own tuple's surface.
...
- With no ground-truth probe available here, a locally ambiguous credential classification stays ambiguous. Report it as unknown rather than blocking on it, and never launch a vendor CLI yourself to settle it.
...
4. Compare comparable-fit candidates on their applicable effective headroom and usable runway.
   Eliminate a candidate only when another candidate dominates it on both dimensions, with at least one strictly better.
...
5. Prefer supported runway evidence that projects availability through the inspectable likely-completion horizon.
   ...
   Preserve projection confidence and basis, the limiting window, and the horizon estimate in the rationale rather than hiding them in a score.
...
8. Older schemas or absent runway and pace fields: do not crash, fabricate runway or pace, treat absence as healthy, or silently exclude a candidate.
