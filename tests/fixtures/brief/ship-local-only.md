...
# Rules
1. Never push to any remote and never open a PR. Work only on your `fm/t1` branch; firstmate handles the merge into local `main`.
...
The task is complete only when committed on your branch `fm/t1`. Do NOT push, do NOT open a PR, do NOT merge.
Keep your branch a clean fast-forward onto the current default branch - if `main` has advanced, rebase onto it so the eventual merge stays a fast-forward.
When it is implemented and committed, append `done: ready in branch fm/t1` to the status file and stop.
